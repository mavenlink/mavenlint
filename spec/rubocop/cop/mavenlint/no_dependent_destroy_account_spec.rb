...
RSpec.describe RuboCop::Cop::Mavenlint::NoDependentDestroyAccount do
  ...
  described_class::ASSOCIATIONS.each do |association|
    it "registers an offense when #{association} :account has dependent: :destroy option" do
      ...
      ruby_code = "#{association} :account, inverse_of: :foo, dependent: :destroy, autosave: true"
